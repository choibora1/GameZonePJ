...
<c:if test="${not empty one}">
   <table>
      <tr height="40">
         <th bgcolor="DarkKhaki" style="text-align: center;">글 번호</th>
         <td>${one.seq}</td>
         <td><input type="text" value="${one.seq}" id="seq" hidden><td>
      </tr>
      
      <tr height="40">
         <th bgcolor="DarkKhaki" style="text-align: center;">아이디</th>
         <td>${one.id}</td>
      </tr>
      
      <tr height="40">
         <th bgcolor="DarkKhaki" style="text-align: center;">제목</th>
         <td>${one.title}</td>
      </tr>
      
      <tr height="40">
         <th bgcolor="DarkKhaki" style="text-align: center;">내용</th>
         <td> <textarea rows="5" cols="50" readonly>${one.content}</textarea></td>
      </tr>
      
      <tr height="40">
         <th bgcolor="DarkKhaki" style="text-align: center;">작성일자</th>
         <td>${one.regdate}</td>
      </tr>
...
<%--    <div id="writeReply" class="searchReply" name="replylist">
      <div>
            <c:if test="${not empty loginID}">
               <!-- textarea -->
               <textarea rows="3" cols="100" name="content" id="replyContent"></textarea>
            <!-- 등록 -->   
         ...
         <ul id="replyaddlist">
            ...
            <c:forEach var="reply" items="${list}" varStatus = "status">
               <div id = "acontent${status.count}">
                  <ul>
                     <li class = "reply${status.count}">
                        ${reply.id} | ${reply.regdate}
                     </li>
                     <li class = "reply${status.count}" id="replyContentLi${status.count}">
                        ${reply.content}
                     </li>
                  </ul>   
               </div>
               <input class = "reply${status.count}" id="replyNumber${status.count}" value="${reply.reply_number}" name="${reply.reply_number}" hidden>
                 <div id="abt${status.count}">
                     <c:if test="${loginID == one.id || loginID == 'admin'}">
                        <input class = "reply${status.count}" type="button" value="수정" id="modifyReply${status.count}">
                        <input class = "reply${status.count}" type="button" value="삭제" id="deleteReply${status.count}">
                     </c:if>
               </div>   
                  </c:forEach>
            </ul>
   ...
   <div id="writeReply" class="searchReply" name="replylist">
      <div>
         <c:if test="${not empty loginID}">
            <!-- textarea -->
            <textarea rows="3" cols="94" name="content" id="replyContent" placeholder="명예훼손, 개인정보 유출, 분쟁 유발, 허위사실 유포 등의 글은 법률에 의해 처벌받을 수 있습니다. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;건전한 커뮤니티를 위해 자제를 당부드립니다."></textarea>
            <!-- 등록 -->
            <input type="button" value="등록" class="sclick">
         </c:if>
      </div> <!-- 댓글 폼 -->
