...
public class PageMaker {
	...
	/*  ※ 주의 필드명이 ePageNo처럼 두 번째 알파벳이 대문자인 경우 ※
	   => setter, getter는 setePageNo, getePageNo 형태로 만들어지기 때문에
	      Lombok.. 등등 사용 시 불편 -> 그러므로 대.소문자 섞어 사용 시(Camel 표기법) 주의. */
	...
	/*   => n개씩 출력한다면 epageNo는 늘 n의 배수   
            displayPageNo = 3이면 3, 6, 9, 12, ......
         => ex) 17 page 요청, displayPageNo = 3일 때 17이 몇 번째 그룹인지 계산하려면,
                17 / 3 -> 5 나머지 2, 결론은 정수 나누기 후 나머지가 있으면 + 1 이 필요함
             -> Math.ceil(17 / 3) -> Math.ceil(5.73..) -> 6.0 -> 6번째 그룹 16, 17, 18
	...
	/* ** 패키지 org.springframework.web.util
	   ...
	   => ?currPage=1&rowsPerPage=5 이것을 만들어줌
...
//------------------------------------------------------------------------------------------------------
	...
	// => ?currPage=1&rsPerPage=3&searchType=t&keyword=Java
